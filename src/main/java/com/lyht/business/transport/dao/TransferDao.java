...
public class TransferDao extends HibernateBaseDao<Transfer> {
	...
	public PageResults getNumber(String num) {
		...
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT HANDOVER AS HANDOVER FROM MATE_RECEIPT WHERE 1=1 ");
		sql.append(" AND HANDOVER LIKE '" + num + "%'");
		sql.append(" ORDER BY HANDOVER DESC");
		PageResults retValue = new PageResults();
	...
	public void update(String ids, String handover) {
		String sql = "update  Delivery SET HANDOVER='"+handover+"' WHERE ID ='"+ids+"' ";
		Query q = this.getSession().createQuery(sql);  
		q.executeUpdate();
	}
