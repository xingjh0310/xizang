(function($) {
	...
	$.System_material = function(option) {
		...
		var opt_url={
		   ...
		   ,url_edit			:basePath+"refund/refund!edit.action"					//获取审核数据
		   ,url_flag			:basePath+"refund/refund!flag.action"					//执行审核功能
		...
		this.InitData=function(opt){
			...
			function queryParams(params) {  // 配置参数
				...
				var opt_parms={
					 ...
					 "refundFormBean.refund.upState"		:"1",
...
//			    	alert(temp);
		...
		this.InitAddEditDel=function(opt){
			opt_all=$.extend(opt_all,opt);
			opt_all.btn_adopt.bind("click",event_adopt)
			opt_all.btn_refuse.bind("click",event_refuse);
			opt_all.btn_record.bind("click",event_record);
			opt_all.btn_ref.bind("click",event_ref);
			opt_all.btn_audit_ok.bind("click",audit_ok);
			opt_all.btn_audit_no.bind("click",audit_no);
		...
		function LoadEditData(row){
			...
			comm_loadFormData_flag_html(row,"_list");// 获取到数据，显示在界面上 退库退料信息
				...
				common_ajax(null, opt_all.url_materialData, function(response){
					...
					for(var i=0;i<res.detail.length;i++){//物资明细
							...
							var material_data = response.materials;
								...
								html += "<td style='padding: 0px;'><input style='height:30px;border: 0px;text-align: right;' data-type='text' class='form-control materiel-num' onkeyup='countTotalPrice()' value='"+detailData.USENUM+"' name='useNum'></td>";
								html += "<td style='padding: 0px;'><input style='height:30px;border: 0px;background-color: #fff;text-align: right;'  data-type='text' class='form-control price' value='"+(detailData.RETURNNUM==null?"":detailData.RETURNNUM)+"' name='returnNum'></td>";
		...
		function event_adopt(){
		 var ids=g_select_and_tip(opt_all.table,"ID");
           ...
           confirm("退库退料","您确定要审核该条记录吗？","icon-info", function(result) {
			   if(result){
				   audit(ids,true);
               }
			}); 
		...
		function event_record(){
			window.location.href = basePath+"business/refund/auditRecord/record.jsp";
		}
		//绑定新增
		function event_add(){
			_edit(0);
...
//			opt_all.info_form.data('bootstrapValidator').resetForm(false);
		...
		function audit(ids,type){
			...
			if(type){
				...
				console.log("不同意"+state)
			}
			var explain =$("#explain").val();
			var url = opt_all.url_flag+"?refundFormBean.ids="+ids+"&refundFormBean.refund.state="+state+"&refundFormBean.refund.explain="+explain;
				 common_ajax(null, url, function(response){
					// 删除后，从后台取出返回数据
		...
		function filesUpload(id){
			...
			file_options= {
				//初始化参数
				url:basePath+"file/upload!saveFileUpload.action?tableName=BILLINFO&tablePkColumn="+id	//文件上传地址
			   ,filters:{//文件过滤器
				   	//上传类型
				    mime_types: [
