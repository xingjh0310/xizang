(function($) {
	$.System_SysDictQ = function(option) {
		//option自定义参数覆盖
		//A、界面控件变量
		var opt_control={
			table 			:$('#tbinfo_ListnmSysDict')		//BootStrapTable的ID
		}
		//B、数据状态参数变量
		var opt_state={
		...
		//加载分类数据ctl_select:下拉框控件，callback-回调函数
		this.Load_EditSelectData= function(dictcate,ctl_select,callback){
			var url=basePath+"system/sysdict!list.action";
			var parms=$.extend({},g_SelectParms,
					{
					   "formBean.infoBean.flag":1
					  ,"formBean.infoBean.sysflag":""
					  ,"formBean.infoBean.listnmSysDictCate":dictcate  ////分类名称
					}
			);
			//加载下拉框数据
			common_ajax(parms,url, function(response) {
				...
				for (var i = 0 ; i< rows.length;i++){
					var html = "<option value='"+rows[i].CODE+"'>"+rows[i].NAME+"</option>";
					ctl_select.append(html);
				}
			});
		}
