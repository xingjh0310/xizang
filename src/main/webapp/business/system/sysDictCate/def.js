(function($) {
	$.System_Sysdictcate = function(option) {
		//option自定义参数覆盖
		//A、界面控件变量
		var opt_control={
		...
		//C、请求地址URL
		var opt_url={
			 url_list		:basePath+"system/sysdictcate!list.action"			//查询数据URL
			,url_edit		:basePath+"system/sysdictcate!edit.action"			//编辑数据URL
			,url_save		:basePath+"system/sysdictcate!save.action"			//保存数据URL
			,url_remove		:basePath+"system/sysdictcate!removeids.action"		//删除数据URL
			,url_flag 		:basePath+"system/sysdictcate!flag.action"			//审核数据URL
		}
		//全部变量，自定义的覆盖默认变量
		...
		function queryParams(params) {  // 配置参数
		    ...
		    var temp = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		      ...
		      ,"formBean.searchName"		: opt_control.searchInput.val() // 查询关键字
					// 在此增加查询条件
		      ,"formBean.infoBean.flag"		: opt_control.select_flag.val() 
		...
		function LoadEditData(id,onlyread){
			...
			common_ajax(null,url, function(response) {
				// 获取到数据，显示在界面上
				comm_loadFormData_flag(response.infoBean,"_");
	            var title="信息维护";
				if (id==0) {
