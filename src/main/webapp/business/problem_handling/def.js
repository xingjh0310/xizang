(function($) {
	...
	$.System_Bank = function(option) {
		...
		//C、请求地址URL
		var opt_url={
			 url_list		:basePath+"question/handler!queryQuestionHandler.action"//查询数据URL
			,url_save		:basePath+"question/handler!saveHandler.action"			//保存数据URL
			,url_downLoad	:basePath+"question/handler!downLoadHandlerInfo.action"	//导出数据URL
		}
...
//			    	alert(temp);
		...
		function LoadEditData(id,flag){
			...
			common_ajax(data,url, function(response) {
				...
				var title="";
				if(flag== null || flag == ""){
					title="<i class='icon icon-pencil'></i> 物资问题处理";
...
//					window.location.reload();
		...
		function downLoadAllDemand(){
			...
			var data="fSubmitFormBean.pageBean.limit="+g_treelist_size
					...
					+"&fSubmitFormBean.mSubmit.engineerCode="+sessionStorage.getItem("engineeringNm");
			confirm("<i class='icon icon-reply'></i>&nbsp;问题处理","您确定要导出全部信息吗？","icon-info", function(result) {
				   if(result){
		...
		function downLoadDemandInPage(){
			...
			if (bt_ids.length == 0) {
				var data_page="fSubmitFormBean.pageBean.limit="+TableQueryParams.limit
							 ...
							 +"&fSubmitFormBean.searchName="+opt_all.searchInput.val()
							 +"&fSubmitFormBean.mSubmit.engineerCode="+sessionStorage.getItem("engineeringNm");
				confirm("<i class='icon icon-circle-arrow-up'></i>&nbsp;问题处理","您确定要导出当前页中的信息吗？","icon-info", function(result) {
					   if(result){
				...
				confirm("<i class='icon icon-circle-arrow-up'></i>&nbsp;问题处理","您确定要导出选中的信息吗？","icon-info", function(result) {
					   if(result){
						   window.location.href=url+data;
					   }
				});
