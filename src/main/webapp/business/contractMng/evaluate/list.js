...
function FMT_file(value,row){
	if(value == null){
		return "<a href='javascript:query.uploading(1)'>"+0+"</a>";
	}
	return "<a href='javascript:query.uploading(1)'>"+value+"</a>";
}
function FMT_evaluate(value,row){
	if(value == 1){
		return "<font style='color:#ea644a'>不满意</font>";
	}else if(value == 2){
		return "<font style='color:#f1a325'>还行</font>";
	}else if(value == 3){
		return "<font style='color:#03b8cf'>一般</font>";
	}else if(value == 4){
		return "<font style='color:#3280fc'>满意</font>";
	}else if(value == 5){
		return "<font style='color:#38b03f'>很满意</font>";
	}
}
//操作--设备维修
function FMT_handle(value,row) {
    var html="";
    html+="<a href='#' onclick='javascript:query.del("+value+")'>" +
