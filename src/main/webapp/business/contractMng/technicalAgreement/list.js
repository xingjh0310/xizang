...
function FMT_handle(value,row) {
    ...
    var html="";
    if(count==1){
    	html+="<a href='javascript:query.edit(1)'>"+
		"<button class='btn btn-xs btn-primary'>"+
		"<div class='visible-md visible-lg'><i class='icon icon-pencil'></i>&nbsp;协议日期调整</div>"+
		"<div class='visible-xs visible-sm'><i class='icon icon-pencil'></i></div>"+
		"</button></a>" ; 
    }else{
    	html+="<a href='javascript:query.edit(1)'>" +
 		"<button class='btn btn-xs btn-success'>" +
 		"<div class='visible-md visible-lg'><i class='icon icon-pencil'></i>&nbsp;技术协议签订</div>" +
 		"<div class='visible-xs visible-sm'><i class='icon icon-pencil'></i></div>" +
 		"</button></a>" ; 
    }
    return html;
