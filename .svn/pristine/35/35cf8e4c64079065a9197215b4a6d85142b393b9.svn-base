(function($) {
	$.System_Sysdict = function(option) {
		//option自定义参数覆盖
		//A、界面控件变量
		var opt_control={
		...
		//C、请求地址URL
		var opt_url={
			 url_list		:basePath+"system/sysdict!list.action"			//查询数据URL
			,url_edit		:basePath+"system/sysdict!edit.action"			//编辑数据URL
			,url_save		:basePath+"system/sysdict!save.action"			//保存数据URL
			,url_remove		:basePath+"system/sysdict!removeids.action"		//删除数据URL
		}
		//全部变量，自定义的覆盖默认变量
		...
		function queryParams(params) {  // 配置参数
		    ...
		    var temp = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
              ...
              ,"formBean.infoBean.listnmSysDictCate":$("#S_ListnmSysDictCate").val()
		    };
		...
		function LoadEditData(id,onlyread){
			...
			common_ajax(null,url, function(response) {
				// 获取到数据，显示在界面上
				comm_loadFormData_flag(response.infoBean,"__");
	            var title="信息维护";
				...
				//初始化选择-字典分类
				$("#listnmSysDictCate").val($("#S_ListnmSysDictCate").val());
				
				$('#listnmSysDictCate').trigger('chosen:updated');//更新选项，此项需要执行，否则chosen更新不了
		...
		function _save() {
			// 参数需要保存的表单，保存url,需要更新的bootstrapTable,必须设置uniqueId: "id"
	 		...
	 		common_ajax(json, opt_all.url_save, function(response){
				...
				var str;
				//取下拉框的文本后，去掉前面()，剩下的文本
				str=$("#listnmSysDictCate").find("option:selected").text();
				str=str.substring(str.indexOf(")")+1);
		    	info=$.extend(info,{"SysDictCateName":str});
...
////////////////////////////////////////////////////////////////////////////////
	...
	function Load_EditSelectData(callback){
		//所有编辑页面下拉框加载
        $('#listnmSysDictCate').empty();
		Load_EditSelectData_ListnmSysDictCate( $('#listnmSysDictCate'),function(){

                comm_chose_init($('#listnmSysDictCate'));
				callback();
		})
		;
	}
    
	//加载字典分类数据ctl_select:下拉框控件
	function Load_EditSelectData_ListnmSysDictCate(ctl_select,callback){
		var url=basePath+"system/sysdictcate!list.action";
