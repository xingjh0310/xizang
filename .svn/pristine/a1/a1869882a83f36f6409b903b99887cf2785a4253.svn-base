...
function querySupplyPlan(){
	...
	common_ajax(data, url, function(response){
		...
		for(i=0;i<supplyPlan.length;i++){
			html+="<option value='"+supplyPlan[i].SUPPLIERPLANCODE+"'>"+supplyPlan[i].supply_plan_title+"</option>";
...
function FMT_State(value,row){
	...
	if(row.PLANSTATE==1 && row.AUDITSTATE==0){
		{html="已上报"}
	}
	if(row.PLANSTATE==1 && row.AUDITSTATE==1){
		{html="已通过"}
	}
	if(row.PLANSTATE==1 && row.AUDITSTATE==2){
		{html="未通过"}
	}
...
function FMT_Oper(value,row) {
    ...
    if(row.PLANSTATE==0){
    	 ...
    	 html="<a href='#' onclick='javascript:arrival.edit("+row.ID+")'>" +
    		...
    		"</button></a>" +
    		"&nbsp;&nbsp;" +
    		"<a href='#' onclick='javascript:arrival.del("+row.ID+")'>" +
    		...
    		"</button></a>&nbsp;&nbsp;<a href='#' onclick='javascript:arrival.up_reported("+row.ID+")'>" +
    	    "<button class='btn btn-xs btn-primary'>" +
    	    "<div class='visible-md visible-lg'><i class='icon icon-circle-arrow-up'></i>&nbsp;上报</div>" +
    	    "<div class='visible-xs visible-sm'><i class='icon icon-circle-arrow-up'></i></div>" +
    	    "</button></a>"+
    		"<br>"+
    	    "<div style='margin-top:5px;'><a href='#' onclick='javascript:arrival.up_time("+row.ID+")'>" +
    	    "<button class='btn btn-xs btn-primary'>" +
    	    "<div class='visible-md visible-lg'><i class='icon icon-refresh'></i>&nbsp;协议、图纸时间调整</div>" +
