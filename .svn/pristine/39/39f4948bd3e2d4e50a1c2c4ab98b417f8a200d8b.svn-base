(function($) {
	$.System_Bank = function(option) {
		...
		//C、请求地址URL
		var opt_url={
			 url_list		:basePath+"question/submit!queryAllQuestion.action"	//查询数据URL
			,url_edit		:basePath+"question/submit!queryAllQuestion.action"	//编辑数据URL
			,url_save		:basePath+"question/submit!saveSubmit.action"	//保存数据URL
			,url_remove		:basePath+"question/submit!removeids.action"		//删除数据URL
			,url_downLoad	:basePath+"question/submit!downLoadSubmitInfo.action"	//导出数据URL
		}
		...
		this.InitData=function(opt){
			...
			function queryParams(params) {  // 配置参数
			    ...
			    var temp = {   // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			      ...
			      ,"fSubmitFormBean.pageBean.sortOrder"	: "asc"// 排位命令（desc，asc）
			    };
...
//			    	alert(temp);
		...
		function LoadEditData(id,onlyread){
			...
			common_ajax(data,url, function(response) {
				...
				var title="";
				if (id==0) {
					title="<i class='icon icon-plus-sign'></i> 新增物资问题情况";
					$("input").val("");
					$("textarea").val("");
					$("#title_div").hide();
					$("#STATE").val(0);
					FILENM=response.fileNm;
					$("#FILENM").val(FILENM);
					$("#DEPTNAME").val(response.mSysDept.name);//上报单位
					$("#STAFFNAME").val(response.mSysStaff.name);//上报人
					$("#TREENMSYSDEPT").val(response.mSysDept.nm);//上报单位nm
					$("#REPORTPERSON").val(response.mSysStaff.nm);//上报人nm
					$("#ENGINEERCODE").val(sessionStorage.getItem("engineeringNm"));//工程nm
				}else{
					title="<i class='icon icon-pencil'></i> 修改物资问题情况";
					$("#title_div").show();
					FILENM=response.rows[0].FILENM;
				}
				fileUploadInie(FILENM,id);
	    ...
	    // 保存数据
		function _save() {
			$("#materielName").val($("#MATERIELCODE option:selected").text());
			$("#queryName").val($("#PROBLEMTYPE option:selected").text())
		...
		function _removeids(){
           ...
           var url= opt_all.url_remove+"?fSubmitFormBean.ids="+ids
			confirm("物资问题上报","您确定要删除该条记录吗？","icon-remove-sign", function(result) {
			   if(result){
		...
		function _del(id){
			...
			var url= opt_all.url_remove+"?fSubmitFormBean.ids="+id;
			confirm("物资问题上报","您确定要删除该条记录吗？","icon-remove-sign", function(result) {
				   if(result){
					   common_ajax(null, url, function(response){
						    _refresh();
		...
		this.upload_file=function(id){
			var fileNm=$("#fileNm_id"+id).val();
			filesUpload(fileNm,id);
			upload_model_show();
		}
		...
		//文件上传
		function filesUpload(fileNm,id){
			var file_options;
			var url_queryFileUpload=basePath+"file/upload!queryFileUpload.action?tableName=question_submit&tablePkColumn="+fileNm;
			common_ajax_async(null,url_queryFileUpload,function(response){
	 			file_options = {
	 					//初始化参数
	 					url:basePath+"file/upload!saveFileUpload.action?tableName=question_submit&tablePkColumn="+fileNm+"&engineerCode="+sessionStorage.getItem("engineeringNm")	//文件上传地址
	 					,filters:{//文件过滤器
	 						// 最大上传文件为 10MB
	 						max_file_size: '10mb'
	 					}
		...
		//详情模态框
		function list(row){
			var url_queryFileUpload=basePath+"file/upload!queryFileUpload.action?tableName=question_submit&tablePkColumn="+row.FILENM;
			common_ajax(null, url_queryFileUpload, function(response){
				comm_loadFormData_flag_html(row,'_quertion');
				var fileUrlList=response.mFileUploadList;
				var html="";
				$("#photo_quertion").empty();
				for(i=0;i<fileUrlList.length;i++){//将问题图片循环遍历到页面中
		...
		function downLoadAllDemand(){
			...
			var data="fSubmitFormBean.pageBean.limit="+g_treelist_size
					+"&fSubmitFormBean.pageBean.offset=0" 
					+"&fSubmitFormBean.pageBean.sortOrder="+TableQueryParams.order
					+"&fSubmitFormBean.pageBean.sort="+TableQueryParams.sort
					+"&fSubmitFormBean.searchName="+opt_all.searchInput.val()
					+"&fSubmitFormBean.mSubmit.engineerCode="+sessionStorage.getItem("engineeringNm");
			confirm("<i class='icon icon-reply'></i>&nbsp;物资问题","您确定要导出全部信息吗？","icon-info", function(result) {
				   if(result){
		...
		function downLoadDemandInPage(){
			...
			if (bt_ids.length == 0) {
				var data_page="fSubmitFormBean.pageBean.limit="+TableQueryParams.limit
							 +"&fSubmitFormBean.pageBean.offset="+TableQueryParams.offset
							 +"&fSubmitFormBean.pageBean.sortOrder="+TableQueryParams.order
							 +"&fSubmitFormBean.pageBean.sort="+TableQueryParams.sort
							 +"&fSubmitFormBean.searchName="+opt_all.searchInput.val()
							 +"&fSubmitFormBean.mSubmit.engineerCode="+sessionStorage.getItem("engineeringNm");
				confirm("<i class='icon icon-circle-arrow-up'></i>&nbsp;物资问题","您确定要导出当前页中的信息吗？","icon-info", function(result) {
					   if(result){
				...
				ids = ids.substring(1);
				var data="fSubmitFormBean.ids="+ids
						+"&fSubmitFormBean.pageBean.limit="+g_treelist_size
						+"&fSubmitFormBean.pageBean.offset=0" 
						+"&fSubmitFormBean.pageBean.sort=ID" 
						+"&fSubmitFormBean.pageBean.sortOrder=DESC";
				confirm("<i class='icon icon-circle-arrow-up'></i>&nbsp;物资问题","您确定要导出选中的信息吗？","icon-info", function(result) {
					   if(result){
						   window.location.href=url+data;
					   }
				});
		...
		//新增/修改时附件上传 初始化
		function fileUploadInie(fileNm,id){
			var file_options;
			var url_queryFileUpload=basePath+"file/upload!queryFileUpload.action?tableName=question_submit&tablePkColumn="+fileNm;
			common_ajax_async(null,url_queryFileUpload,function(response){
	 			file_options = {
